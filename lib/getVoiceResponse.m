function [RT] = getVoiceResponse(threshold, time, filename, paHandle, varargin)
...
toc
%% Record the signal
tic;disp('preallocate')
% Preallocate an internal audio recording  buffer with a x seconds:
PsychPortAudio('GetAudioData', paHandle, 5);
toc
tic;disp('wait/record loop')
timePoint2 = GetSecs;
...
tic;disp('stop')
PsychPortAudio('Stop', paHandle);toc
tic; disp('get recording')
audioData = PsychPortAudio('GetAudioData', paHandle, [], [], [], 1);
toc
tic;disp('other post trial actions')
...
if ~isempty(filename) % If no file name is provided, nothing is saved.
    ...
    elseif saveMode == 2 % Only .wav
        % Save .wav file
        audiowrite([filename '.wav'], transpose(audioData), 44100, 'BitsPerSample', 16)
